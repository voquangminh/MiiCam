/**
 * @file encode_with_capture_tamper_detection2.c
 *
 * Support platform: GM8210, GM8287, GM8139
 * This sample code is demo tamper detection by capture histogram
 * 
 ...
 */
/**
 * @example encode_with_capture_tamper_detection2.c
 */
...
void my_notify_handler(gm_obj_type_t obj_type, int vch, gm_notify_t notify)
{
    if (notify == GM_NOTIFY_TAMPER_ALARM)
        printf("Ch(%d) tamper alarm!\n", vch);
    else if (notify == GM_NOTIFY_TAMPER_ALARM_RELEASE)
        printf("Ch(%d) tamper release!\n", vch);
}
...
static void *encode_thread(void *arg)
{
    ...
    mv_data =  (char *)malloc(MD_DATA_LEN);
...
int main(void)
{
    ...
    cap_tamper.tamper_sensitive_b = 20;
    cap_tamper.tamper_threshold = 80; 
    cap_tamper.tamper_sensitive_h = 0;
    gm_set_cap_tamper(ch, &cap_tamper);
    gm_register_notify_handler(GM_NOTIFY_TAMPER_ALARM, my_notify_handler);
    gm_register_notify_handler(GM_NOTIFY_TAMPER_ALARM_RELEASE, my_notify_handler);
