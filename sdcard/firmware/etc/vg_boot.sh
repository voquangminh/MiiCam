...
case "$video_frontend" in
    ...
    "ov9732")
        ...
        if [ "$video_system" == "NTSC" ]  ; then
            ...
            insmod /lib/modules/3.3.0/fisp_algorithm.ko exp_us=1 pwr_freq=0
            insmod /lib/modules/3.3.0/fisp_ov9732.ko sensor_w=1280 sensor_h=720 fps=30
        elif [ "$video_system" == "PAL" ] ; then
		insmod /lib/modules/3.3.0/fisp328.ko cfg_path=${CONFIG_PARTITION}/isp328_ov9732.cfg
		insmod /lib/modules/3.3.0/fisp_algorithm.ko exp_us=1 pwr_freq=1
		insmod /lib/modules/3.3.0/fisp_ov9732.ko sensor_w=1280 sensor_h=720 fps=30
...
MANUFA="/mnt/media/mmcblk0p1/manufacture.bin"
WRITE_NUM="/mnt/media/mmcblk0p1/write_num.bin"
TEST_FOCUS="/mnt/media/mmcblk0p1/test_focusing.bin"
SDCARD_DIR="/mnt/media/mmcblk0p1"

#for manufature
if [ -f "$MANUFA" ] && [ ! -f "$WRITE_NUM" ] && [ ! -f "$TEST_FOCUS" ] ;
then
	tar -xf "$MANUFA" -C /tmp/
	cd /tmp/manufacture
	./test_drv &
#for write_num
elif [ ! -f "$MANUFA" ] && [ -f "$WRITE_NUM" ] && [ ! -f "$TEST_FOCUS" ] ;
then
	tar -xf "$WRITE_NUM" -C /tmp/
	cd /tmp/write_num
	./write_num &
#for test_focusing
elif [ ! -f "$MANUFA" ] && [ ! -f "$WRITE_NUM" ] && [ -f "$TEST_FOCUS" ] ;
then
	tar -xf "$TEST_FOCUS" -C /tmp/
