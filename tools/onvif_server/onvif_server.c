...
static void handle_soap(const char*r,char*out,size_t size){out[0]=0;append(out,size,"%s",SOAP_HEAD);
 ...
 else if(strstr(r,"GotoPreset")){char token[64];int p;if(xml_tag(r,"tptz:PresetToken",token,sizeof(token)) == 0){for(p=0;p<PRESET_MAX;p++){if(ptz.presets[i].used && strcmp(ptz.presets[i].token,token) == 0){int mdx = ptz.presets[i].x - ptz.x;int mdy = ptz.presets[i].y - ptz.y;motor_move_relative(mdx, mdy);break;}}}append(out,size,"<tptz:GotoPresetResponse/>");}
 else if(strstr(r,"AbsoluteMove")){float x, y;int tx, ty;int adx, ady;log_message("PTZ","AbsoluteMove request:%s",r);if(xml_attr_float(r,"PanTilt","x",&x) < 0 || xml_attr_float(r,"PanTilt","y",&y) < 0){log_message("PTZ","cannot parse PanTilt");soap_fault(out,size,"Invalid PanTilt");return;}tx = pan_to_x(x);ty = tilt_to_y(y);adx = tx - ptz.x;ady = ty - tz.y;motor_stop();if(motor_move_relative(adx, ady) < 0){log_message("PTZ","motor_move_relative failed errno=%d (%s)",errno,strerror(errno));soap_fault(out,size,"Motor failure");return;}append(out,size,"<tptz:AbsoluteMoveResponse/>");} 
