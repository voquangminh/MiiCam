...
static void handle_soap(const char*r,char*out,size_t size){out[0]=0;append(out,size,"%s",SOAP_HEAD);
 ...
 else if(strstr(r,"RelativeMove")){log_message("PTZ","RelativeMove request:%s",r);float x,y;if(xml_attr_float(r,"PanTilt","x",&x)<0||xml_attr_float(r,"PanTilt","y",&y)<0){soap_fault(out,size,"Invalid translation");return;}motor_refresh();pthread_mutex_lock(&motor_mutex);int dx, dy;dx = (x >= 0.0f) ? (int)(x * X_MAX + 0.5f) : (int)(x * X_MAX - 0.5f);dy = (y >= 0.0f) ? (int)(y * Y_MAX + 0.5f) : (int)(y * Y_MAX - 0.5f);tx = clampi(ptz.x + dx, 0, X_MAX);ty = clampi(ptz.y + dy, 0, Y_MAX);pthread_mutex_unlock(&motor_mutex);motor_stop();log_message("PTZ","relative tx=%d ty=%d",tx,ty);if(motor_goto(tx,ty)<0){soap_fault(out,size,"Motor failure");return;}append(out,size,"<tptz:RelativeMoveResponse/>");}
