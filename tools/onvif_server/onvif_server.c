...
else{soap_fault(out,size,"Action not supported");return;}append(out,size,"%s",SOAP_TAIL); else if(strstr(r,"SetYellowLEDBlink")){int on_ms = 500;int off_ms = 500;get_int_tag(r,"OnTime",&on_ms);get_int_tag(r,"OffTime",&off_ms);on_ms = clampi(on_ms,50,60000);off_ms = clampi(off_ms,50,60000);if(yellow_led_blink(on_ms,off_ms) < 0){soap_fault(out,size,"Yellow LED blink failed");return;}append(out,size,"<tmd:SetYellowLEDBlinkResponse/>");}
 else if(strstr(r,"SetYellowLED")){int v;if(get_int_tag(r,"Enabled",&v) < 0 || yellow_led_set(v != 0) < 0){soap_fault(out,size,"Yellow LED failed");return;}append(out,size,"<tmd:SetYellowLEDResponse/>");} else if(strstr(r,"SetIrCut")){int v;if(get_int_tag(r,"Enabled",&v)<0||ircut_set(v)<0){soap_fault(out,size,"IR-cut failed");return;}append(out,size,"<tmd:SetIrCutResponse/>");}
