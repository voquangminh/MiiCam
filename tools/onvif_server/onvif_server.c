...
static void handle_soap(const char*r,char*out,size_t size){out[0]=0;append(out,size,"%s",SOAP_HEAD);
 ...
 else if(strstr(r,"GetPresets")){int i;append(out,size,"<tptz:GetPresetsResponse>");pthread_mutex_lock(&motor_mutex);for(i = 0; i < PRESET_MAX; i++){if(!ptz.presets[i].used)continue;append(out,size,"<tptz:Preset token=\"%s\">""<tt:Name>%s</tt:Name>""<tt:PTZPosition>""<tt:PanTilt x=\"%.4f\" y=\"%.4f\"/>""</tt:PTZPosition>""</tptz:Preset>",ptz.presets[i].token,ptz.presets[i].name,x_to_pan(ptz.presets[i].x),y_to_tilt(ptz.presets[i].y));}pthread_mutex_unlock(&motor_mutex);append(out,size,"</tptz:GetPresetsResponse>");}
 ...
 else if(strstr(r,"GotoPreset")){char token[64];int i;if(xml_tag(r,"tptz:PresetToken",token,sizeof(token)) == 0){for(i=0;i<PRESET_MAX;i++){if(ptz.presets[i].used && strcmp(ptz.presets[i].token,token) == 0){int dx = ptz.presets[i].x - ptz.x;int dy = ptz.presets[i].y - ptz.y;motor_move_relative(dx, dy);break;}}}append(out,size,"<tptz:GotoPresetResponse/>");}
 else if(strstr(r,"AbsoluteMove")){log_message("PTZ","AbsoluteMove request:%s",r);float x,y;if(xml_attr_float(r,"PanTilt","x",&x)<0||xml_attr_float(r,"PanTilt","y",&y)<0){log_message("PTZ","cannot parse PanTilt");soap_fault(out,size,"Invalid PanTilt");return;}log_message("PTZ","absolute x=%f y=%f",x,y);motor_stop();int dx = ptz.presets[i].x - ptz.x;int dy = ptz.presets[i].y - ptz.y;motor_move_relative(dx, dy);int tx = pan_to_x(x);int ty = tilt_to_y(y);int dx = tx - ptz.x;int dy = ty - ptz.y;if(motor_move_relative(dx,dy) < 0){soap_fault(out,size,"Motor failure");return;}{log_message("PTZ","motor_move_relative failed errno=%d (%s)",errno,strerror(errno));soap_fault(out,size,"Motor failure");return;}append(out,size,"<tptz:AbsoluteMoveResponse/>");}
 else if(strstr(r,"RelativeMove")){log_message("PTZ","RelativeMove request:%s",r);float x,y;if(xml_attr_float(r,"PanTilt","x",&x)<0||xml_attr_float(r,"PanTilt","y",&y)<0){soap_fault(out,size,"Invalid translation");return;}motor_refresh();pthread_mutex_lock(&motor_mutex);int tx, ty, dx, dy;dx = (x >= 0.0f) ? (int)(x * X_MAX + 0.5f) : (int)(x * X_MAX - 0.5f);dy = (y >= 0.0f) ? (int)(y * Y_MAX + 0.5f) : (int)(y * Y_MAX - 0.5f);tx = clampi(ptz.x + dx, 0, X_MAX);ty = clampi(ptz.y + dy, 0, Y_MAX);pthread_mutex_unlock(&motor_mutex);motor_stop();log_message("PTZ","relative tx=%d ty=%d",tx,ty);if(motor_move_relative(dx,dy) < 0){soap_fault(out,size,"Motor failure");return;}append(out,size,"<tptz:RelativeMoveResponse/>");}
