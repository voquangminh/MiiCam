...
pthread_t zoom_thread_id      	= 0;		// * Zoom (ONVIF -> OSD)
...
void *groupfd;                    // * Return of gm_new_groupfd(0)
void *bindfd;                     // * Return of gm_bind(0)
void *capture_object;
void *encode_object;
...
struct CommandLineArguments {
    ...
    char osd_text[32];
    /* Audio configuration */
    int audio_sample_rate;    /* e.g., 8000, 16000, 32000, 44100 */
    int audio_bitrate;        /* in bits/sec */
    int audio_frame_samples;  /* AAC frame samples (e.g. 1024) */
    int audio_channel_type;   /* GM_MONO / GM_STEREO */
    int audio_encode_type;    /* GM_AAC / GM_PCM / GM_G711_ALAW / GM_G711_ULAW */
} cliArgs;
...
/* Zoom thread: read ONVIF PTZ state file and update OSD font zoom accordingly */
static void *rtspd_zoom_thread(void *arg)
{
    (void)arg;
    const char *state_file = "/tmp/onvif_ptz.state";
    char buf[256];
    while (rtspd_sysinit) {
        ...
        if (f) {
            if (fgets(buf, sizeof(buf), f)) {
                ...
                if (sscanf(buf, "%d %d %d %d", &x, &y, &hx, &hy) >= 2) {
                    int zoom = GM_OSD_FONT_ZOOM_NONE;
                    int dx = abs(x - hx);
                    if (dx <= 2) zoom = GM_OSD_FONT_ZOOM_NONE;      /* 1x */
                    else if (dx <= 8) zoom = GM_OSD_FONT_ZOOM_2X;  /* 2x */
                    else zoom = GM_OSD_FONT_ZOOM_4X;               /* 4x */
                    if (cliArgs.font_zoom != zoom) {
                        cliArgs.font_zoom = zoom;
                        if (enc_param[0][0].cap.obj)
                            rtspd_update_osd_text(enc_param[0][0].cap.obj);
                    }
                }
            }
            fclose(f);
...
void gm_enc_init(int cap_ch, int cap_path, int rec_track, int enc_type, int mode, int framerate, int bitrate, int width, int height)
{
    ...
    if (param->cap.obj == NULL) {
        ...
        if ((width >= (gm_system.cap[0].dim.width / 2)) &&			// cap[cap_ch]
            (height >= (gm_system.cap[0].dim.height / 2))) {		// cap[cap_ch]
            dnr_attr.enabled = 1;
...
static void audio_init() 
{
    ...
    audio_grab_attr.channel_type = cliArgs.audio_channel_type;
    gm_set_attr(audio_grab_object, &audio_grab_attr);

    audio_encode_attr.encode_type = cliArgs.audio_encode_type;
        ...
        for (i = 0; i < CAP_CH_NUM; i++) {
        	...
        	for (i = 0; i < CAP_CH_NUM; i++) {
                ...
                if ((bs[i][j].retval < 0) && bs[i][j].bindfd)
                    ...
                    if (!sdp_ready) {
                        // generate audio SDP for stream registration
                        stream_sdp_parameter_encoder("AAC", (unsigned char *) multi_bs.bs.bs_buf, multi_bs.bs.bs_len, pb->audio.sdpstr, SDPSTR_MAX);
                        sdp_ready = 1;
                    }
                    if (audio_file == NULL) {
                    ...
                    // Use configured audio sample rate for RTP timestamp scaling
                    entity.timestamp = (multi_bs.bs.timestamp * (unsigned int)cliArgs.audio_sample_rate) / 1000;
                    // AU Headers
                    entity.data[0] = 0;
                    entity.data[1] = 0x10;
...
static int rtspd_start(int port)
{
    ...
    // * Zoom thread to integrate ONVIF PTZ with OSD zoom
...
static void print_usage(void)
{
    ...
    printf(
        ...
        "-A [8000|16000|32000|44100] - Audio sample rate (default: 16000)\n"
        "-R [bits]      - Audio bitrate (default: 16000)\n"
        "-S [samples]   - Audio frame samples (AAC) (default: 1024)\n"
        "-P [1|2]       - Audio channels 1=mono,2=stereo (default: 1)\n"
        "-X [aac|g711alaw|g711ulaw|pcm] - Audio encode type (default: aac)\n"
            ...
            if (argv[i][0] != '-' ) {
                ...
                switch (argv[i][1]) {
                    ...
                    case 'r':
                        ...
                        break;
                    case 'b':
                        cliArgs.bitrate     = atoi(&argv[i][2]);	// * Set bitrate 
                        break;
                    case 'f':
                        cliArgs.framerate   = atoi(&argv[i][2]);	// * Set framerate
                        break;
                    case 'w':
                    ...
                    case 'h':
                        ...
                        break;
                    case 'm':
                        cliArgs.bitrateMode = atoi(&argv[i][2]);	// * Set bitrateMode
                        break;
                    case 'j':
                        cliArgs.encoderType = ENC_TYPE_MJPEG;		// * Set enc type to MJPEG
                        break;
                    case '4':
                        cliArgs.encoderType = ENC_TYPE_MPEG4;		// * Set enc type to MJEG4
                        break;
                    case 'o':
                        cliArgs.osd = 1;							// * Set OSD attribute
                        if (argv[i][2] != '\0') {
                        ...
                        break;
                    case 'B':
                        cliArgs.osd = 1;							// * Set background OSD
                        if (argv[i][2] != '\0') {
                    ...
                    case 'X':
                        {
                            ...
                            if (v) {
                                ...
                                else if (strcasecmp(v, "g711alaw") == 0) cliArgs.audio_encode_type = GM_G711_ALAW;
                                else if (strcasecmp(v, "g711ulaw") == 0) cliArgs.audio_encode_type = GM_G711_ULAW;
