...
int parsing_mv_data(unsigned char *event_buf, struct mv_cfg_t *mv_param,  unsigned int u_width, unsigned int u_height, unsigned int u_mb_width, unsigned int u_mb_height, unsigned int sensitive_th, unsigned int frame_count, unsigned int mb_w_num, unsigned int mb_h_num, int mv_idx)
{
    int ret = 0;
    ...
    ret = transform(mv_param, u_width, u_height, u_mb_width, u_mb_height, sensitive_th, frame_count, mb_w_num, mb_h_num, mv_idx);
    if (ret < 0) {
...
int parsing_mv_initial(unsigned int mb_w_num, unsigned int mb_h_num, int mv_idx)
{
    ...
    if (parsing_user_motion_size[mv_idx] < user_motion_size_in) {
        ...
        kernel_mv_data[mv_idx] = (unsigned char *)malloc(sizeof(unsigned char) * CAP_MOTION_SIZE);
        if (kernel_mv_data[mv_idx] == NULL)
            return -1;
