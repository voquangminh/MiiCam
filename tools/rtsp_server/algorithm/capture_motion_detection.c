 ...
 *
 * @fn int do_capture_motion(gm_cap_md_info_t *cap_motion, struct mdt_user_t *mdt_param, struct mdt_res_t *active, int mdt_idx)
...
*/
static int do_capture_motion(gm_cap_md_info_t *cap_motion, struct mdt_user_t *mdt_param, struct mdt_res_t *active, int mdt_idx)
{
    int ret = 0;
    ...
    if ((mdt_param->mv_param.k_height == cap_motion->md_dim.height) && (mdt_param->mv_param.k_width == cap_motion->md_dim.width) && (cap_motion->is_valid == 1)) {
...
static int capture_motion_update(struct mdt_user_t *mdt_param, struct mdt_res_t *active, int mdt_idx)
{
    ...
    if (user_motion_size[mdt_idx] < user_motion_size_update) {
        ...
        active->active_flag = (unsigned char *) malloc(sizeof(unsigned char) * user_motion_size_update);
        if (active->active_flag == NULL) {
            ret = MOTION_INIT_ERROR;
            goto err_ext;
        }
...
int motion_detection_init(void)
{
    ...
    for (mdt_idx = 0; mdt_idx < MAX_CAP_MD_NUM; mdt_idx++) {
        ...
        mdt_param[mdt_idx].mdt_alg.u_height      = 720;
        mdt_param[mdt_idx].mdt_alg.u_width       = 1280;
        ...
        if (ret < 0) {
            printf("Motion Detection: capture_motion_info_init error at mdt_idx(%d)\n", mdt_idx);
...
int motion_detection_end(void)
{
    ...
    for (mdt_idx = 0; mdt_idx < MAX_CAP_MD_NUM; mdt_idx++) {
        ...
        if (ret < 0) {
            printf("Motion Detection: capture_motion_end error at mdt_idx(%d)\n", mdt_idx);
            ret = -1;
            continue;
        }
    }
...
int motion_detection_update(void *bindfd, struct mdt_alg_t *mdt_alg)
{
    ...
    mb_cell_size_ori = sizeof(unsigned char) * mdt_param[mdt_idx].mdt_alg.mb_w_num * mdt_param[mdt_idx].mdt_alg.mb_h_num;
    mb_cell_size_new = sizeof(unsigned char) * mdt_alg->mb_w_num * mdt_alg->mb_h_num;

    if (mb_cell_size_new == 0) {
        printf("Error, mb_w_num(%d) * mb_h_num(%d) = 0 at bindfd(0x%p)\n", mdt_alg->mb_w_num, mdt_alg->mb_h_num, bindfd);
        ret = -1;
        goto err_ext;
    }
    ...
    memcpy((void *)mdt_param[mdt_idx].mdt_alg.mb_cell_en, (void *)mdt_alg->mb_cell_en, mb_cell_size_new);
...
int motion_detection_handling(gm_multi_cap_md_t *cap_md, struct mdt_result_t *mdt_result, int num_cap_md)
{
    ...
    for (mdt_idx = 0; mdt_idx < MAX_CAP_MD_NUM; mdt_idx++) {
        ...
        result = do_capture_motion(&cap_md[cap_md_idx].cap_md_info, &mdt_param[mdt_idx], &active[mdt_idx], mdt_idx);
        mdt_result[cap_md_idx].bindfd = cap_md[cap_md_idx].bindfd;
