...
static int motion_setup(void)
{
    ...
    mdt_alg.sub_region = (struct mdt_reg_t *) malloc(sizeof(struct mdt_reg_t));
    if (!mdt_alg.sub_region) return -1;
    memset(mdt_alg.sub_region, 0, sizeof(struct mdt_reg_t));

    mdt_alg.u_width      = CFG_WIDTH;
    mdt_alg.u_height     = CFG_HEIGHT;
    mdt_alg.u_mb_width   = MD_MB_SIZE;
    mdt_alg.u_mb_height  = MD_MB_SIZE;
    mdt_alg.training_time = 15;
    mdt_alg.frame_count   = 0;
    mdt_alg.sensitive_th  = 80;   /* do nhay, 0~100 */

    mb_w_num = (mdt_alg.u_width  + (MD_MB_SIZE - 1)) / MD_MB_SIZE;
    mb_h_num = (mdt_alg.u_height + (MD_MB_SIZE - 1)) / MD_MB_SIZE;
    mdt_alg.mb_w_num = mb_w_num;
    mdt_alg.mb_h_num = mb_h_num;

    /* 1 vung quan tam duy nhat = toan bo khung hinh */
    mdt_alg.sub_region[0].is_enabled    = 1;
    mdt_alg.sub_region[0].start_block_x = 0;
    mdt_alg.sub_region[0].start_block_y = 0;
    mdt_alg.sub_region[0].end_block_x   = mb_w_num - 1;
    mdt_alg.sub_region[0].end_block_y   = mb_h_num - 1;
    mdt_alg.sub_region[0].alarm_th      = 80;
    mdt_alg.sub_region[0].alarm         = NO_MOTION;
    mdt_alg.sub_region_num = 1;
    ...
    if (motion_detection_update(venc_bindfd, &mdt_alg) != 0) {
        fprintf(stderr, "[MD] motion_detection_update loi\n");
        return -1;
    }

    mdt_result.sub_region = (struct mdt_reg_result_t *)
                               malloc(sizeof(struct mdt_reg_result_t));
    if (!mdt_result.sub_region) return -1;
    mdt_result.sub_region_num = 1;
...
static void *motion_thread(void *arg)
{
    ...
    while (g_running) {
        ...
        if (mdt_result.ch_result == MOTION_IS_READY) {
            cur_motion = (mdt_result.sub_region[0].reg_result == MOTION_DETECTED) ? 1 : 0;
...
cleanup:
    ...
    if (mdt_alg.sub_region) free(mdt_alg.sub_region);
    if (mdt_result.sub_region) free(mdt_result.sub_region);
