...
int main(int argc, char *argv[]) 
{
    ...
    while((opt = getopt(argc, argv, "D:c:y:g:a:d:n:O:F:vShj:w:1:2:3:")) != -1) {
        switch (opt) {
            ...
            case 'g': 
                nightChangeCmd = optarg;
                break;

            case 'a': 
                readAverageCount = atoi(optarg);
                break;
            ...
            case 'n': 
                ...
                break;
            case 'O': 
                thresholdOn = atof(optarg);
                break;
            case 'F': 
                thresholdOff = atof(optarg);
                break;
            case 'v':
                verbose++;
                break;
  ...
  while(1){
    ...
    if(readIspInfo(&exposure, &iridix, &colortemp) == 3){
      ...
      if(mode_switch_wait_count){
        ...
        if(verbose >= 1) 
            printf("mode switch wait active\n");
        continue;
      }
      ...
      if(nightModeEnabled == false && exposure > eq1_user_exposure){
        if(verbose >= 1) 
            printf("Switching to night new exp=%d\n", exposure);
